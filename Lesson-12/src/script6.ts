...
{
    let names: string[] = ["Ivan", "Petro", "Olga", "Anna", "Danylo", "Maxim"];
    const targetLength = 5;

    // Сортування за довжиною
    function sortByLength(arr: string[]) {
    ...
    function binarySearchByLength(arr: string[], targetLength: number, start: number, end: number): number {
        if (start > end) {
            return -1; 
        }
